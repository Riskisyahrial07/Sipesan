...
class Jurnal_model extends CI_Model{
    ...
    public function getAkunInJurnal(){
        return $this->db->select('transaksi.no_reff,akun.no_reff,akun.nama_reff')
                    ->from($this->table)            
                    ->join('akun','akun.no_reff = transaksi.no_reff ')
    ...
    public function getValidationRules(){
        return [
            ...
            [
                'field'=>'no_reff_second',
                'label'=>'Nama Akun',
                'rules'=>'trim|required'
            ],
            [
                'field'=>'jenis_saldo_second',
                'label'=>'Jenis Saldo',
                'rules'=>'trim|required'
            ],
            [
    ...
    public function getdataKeranjang() {
        ...
        // Inisialisasi array untuk menyimpan data yang akan dikirim ke view
        $dataKeranjang = array();
        ...
        if ($query->num_rows() > 0) {
            ...
            // Hanya ambil atribut-atribut yang ingin ditampilkan di view
            foreach ($resultArray as $row) {

                $tanggalFormatted = date('Y-m-d', strtotime($row['tanggal_transaksi']));

                $dataKeranjang[] = array(
                    'keranjang_idatribut1' => $row['keranjang_id'],
                    'keranjang_pengguna_id' => $row['keranjang_pengguna_id'],
                    'keranjang_total' => $row['keranjang_total'],
                    'keranjang_status' => $row['keranjang_status'],
                    'keranjang_date_created' => $tanggalFormatted,
                    // Tambahkan atribut lain yang ingin ditampilkan di view
                );
            }
        }
    
        // Mengembalikan data yang sudah dipilih atribut-atributnya dalam bentuk array
