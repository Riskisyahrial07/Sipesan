...
class BayarController extends CI_Controller{
	...
	public function keranjang(){
		...
		$data = array(
			...
			'desain' => $this->BayarModel->lihat_keranjang_desain($this->session->userdata('session_id'),'belum',$keranjang['keranjang_id'])->result_array(),
			'title' => 'Keranjang | Visual Creative Agency'
		);
	...
	public function bayar($id){
		if (isset($_POST['selesai'])){
			...
			$dataFaktur = array(
				...
				'faktur_bank' => $bank,
				'faktur_alamat' => $this->input->post('alamat'),
				'faktur_pengambilan' => $this->input->post('pengambilan')
			);
		...
		$data = array(
			...
			'desain' => $this->BayarModel->lihat_keranjang_desain($this->session->userdata('session_id'),'belum',$pesanan['keranjang_id'])->result_array(),
			'title' => 'Pembayaran | Visual Creative Agency'
		);
		$this->load->view('frontend/templates/header',$data);
	...
	public function selesai($bank,$id){
		$dataBank = array(
			'bni' => 'Bank BNI 123456789 Atas Nama Risky Perdana ',
			'bri' => 'Bank BRI 115511026 Atas Nama Risky Perdana '
		);
		$data = array(
			'bank' => $dataBank[$bank],
			'pesanan' => $this->BayarModel->lihat_keranjang_status_selesai($this->session->userdata('session_id'),'bayar_menunggu',$id)->row_array(),
			'title' => 'Terima Kasih | Visual Creative Agency'
		);
		$this->load->view('frontend/templates/header',$data);
	...
	public function konfirmasi($id){
		if (isset($_POST['konfirmasi'])){
			...
			$data = array(
				'title' => 'Konfirmasi Pembayaran | Visual Creative Agency',
