...
//		var_dump($konfirmasi);die;
		$data = array(
			...
			'desain' => $this->BayarModel->lihat_keranjang_desain($transaksi['keranjang_pengguna_id'], 'bayar_menunggu', $transaksi['keranjang_id'])->result_array(),
		);
		$this->load->view('backend/templates/header');
	...
	{
		$faktur = $this->db->get_where('sipesan_faktur', ['faktur_id' => $id])->row_array();
		$orders = $this->db->get_where('sipesan_order', ['keranjang_id' => $faktur['faktur_keranjang_id']])->result();
		...
		foreach ($orders as $row) {
			$form = [
				[
					'id_user' => 1,
					'no_reff' => $this->getRefNumber($row->jenis_pesanan),
			...
			$this->db->insert_batch('transaksi', $form);
		}
	...
	public function email($id)
	{
		...
		$this->email->from('visualcreativeafency2023@gmail.com', 'Visual Creative Agency');
	...
	function getRefNumber($jenis_pesanan) 
	{
		$ref = 0;
